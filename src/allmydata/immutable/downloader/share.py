...
class Share:
    ...
    def __init__(self, bucket, server, verifycap, commonshare, node,
        ...
        server_version = self._server.get_version()
    ...
    def __repr__(self):
        return "Share(sh%d-on-%s)" % (self._shnum,
                                      self._server.get_short_description())
    ...
    def _got_error(self, f, start, length, req_ev, lp):
        ...
        log.msg(format="error requesting %(start)d+%(length)d"
                ...
                start=start, length=length,
                server=self._server.get_short_description(),
                si=self._si_prefix,
